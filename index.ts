...
const white: ChalkInstance = chalk.hex("#FEFAF6");
const lightBrown: ChalkInstance = chalk.hex("#EADBC8");
const brown: ChalkInstance = chalk.hex("#DAC0A3");
const navy: ChalkInstance = chalk.hex("#102C57");
const bgNavy: ChalkInstance = chalk.bgHex("#102C57");
...
let cleanSentence = (input: string): string => {
...
const cleanedSentence: string = cleanSentence(answer.sentence);
...
words.forEach((word: string) => console.log(lightBrown.bold(`${chalk.green.bold("*")} ${word}.`)));
...
const wanTo: {continue: boolean} = await inquirer.prompt(
    {
        name: "continue",
        type: "confirm",
